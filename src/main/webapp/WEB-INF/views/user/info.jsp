							...
							<div class="profileFrame">
								<c:choose>
									...
									<c:when test="${userOneSelect.browniePoint > 300}">
										<img src="${pageContext.request.contextPath}/img/frame/purple.png" title="경험치: ${userOneSelect.browniePoint}/400">
									</c:when>
									<c:when test="${userOneSelect.browniePoint > 200}">
										<img src="${pageContext.request.contextPath}/img/frame/blue.png" title="경험치: ${userOneSelect.browniePoint}/300">
									</c:when>
									<c:when test="${userOneSelect.browniePoint > 100}">
										<img src="${pageContext.request.contextPath}/img/frame/red.png" title="경험치: ${userOneSelect.browniePoint}/200">
									</c:when>
									<c:otherwise>
										<img src="${pageContext.request.contextPath}/img/frame/green.png" title="경험치: ${userOneSelect.browniePoint}/100">
									</c:otherwise>
								</c:choose>
							...
							<c:choose>
								...
								<c:otherwise>
									<div style="position: absolute; top: 20px; left: 240px;">
										<button style="background: black; color: white; margin: 0px;" onclick="fn_SyncTierLv()" title="갱신하기">
						...
						<div class="col">
							...
							<c:choose>
								<c:when test="${recentlyChatUserVOList == null or fn:length(recentlyChatUserVOList) == 0}">
									<div class="nickName">최근 채팅한 플레이어가 없습니다.</div>
								</c:when>
								<c:otherwise>
